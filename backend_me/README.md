...
- **Berkeley Database utils**: db-dump command and java libraries to handle .bin files generated by findimagedupes.
...
```
sudo apt-get install file coreutils findimagedupes db-util libdb5.3-java-dev xnest xvfb x11-xserver-utils
```

For Java to find JNI library for Berkeley db5.3, it is needed to:
```
sudo ln -s /usr/lib/x86_64-linux-gnu/libdb_java-5.3.so /usr/lib
```
